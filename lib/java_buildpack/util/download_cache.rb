...
module JavaBuildpack::Util
  ...
  class DownloadCache # rubocop:disable ClassLength
    ...
    # Delivers the cached file to the given block if and only if this method returns +true+.
    def deliver(uri, immutable_file_cache, &block)
      deliver_cache = cache_ready?(immutable_file_cache, uri)

      immutable_file_cache.data(&block) if deliver_cache
      deliver_cache
    end
